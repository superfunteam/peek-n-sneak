...
const host = (await post('/api/rooms', { level: 'merrimac', expert: false }))
  .data;
assert.ok(host.token);
assert.equal(host.game.players[1].room, 'hidden');
const path = '/api/rooms/' + host.code;
const guest = (await post(path, { action: 'join' })).data;
...
const approach = async (who: number, x: number, y: number) => {
  for (let i = 0; i < 45; i++) {
    ...
    if (d < 5) break;
    await tick({ x: (x - p.x) / d, y: (y - p.y) / d, action: false }, who);
  }
...
for (let i = 0; i < 9; i++) await tick({ x: 0, y: 0, action: true }, 0);
