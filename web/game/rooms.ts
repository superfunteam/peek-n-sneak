...
export async function createRoom(level: LevelId, expert: boolean) {
  ...
  await db
    .prepare(
      'INSERT INTO rooms (code,state,host_token,guest_token,revision,updated_at,expires_at) VALUES (?,?,?,NULL,0,?,?)',
    )
    .bind(code, JSON.stringify(match), hostToken, now, now + DAY)
    .run();
  return {
    token: hostToken,
...
export async function joinRoom(code: string) {
  ...
  const result = await database()
    .prepare(
      'UPDATE rooms SET guest_token = ?, state = ?, revision = revision + 1 WHERE code = ? AND guest_token IS NULL AND revision = ?',
    )
    .bind(guestToken, JSON.stringify(match), code, row.revision)
    .run();
  if (!result.meta.changes)
    throw new RoomError(
      'Someone just joined this room. Ask for a fresh invite.',
      409,
    );
  return {
...
export async function updateRoom(
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    const saved = await database()
      ...
      .bind(JSON.stringify(match), now, code, row.revision)
      .run();
